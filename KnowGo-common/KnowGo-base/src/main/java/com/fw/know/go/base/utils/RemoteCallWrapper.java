...
import com.fw.know.go.base.exception.RemoteCallException;
...
import static com.fw.know.go.base.exception.BizErrorCode.*;


/**
 * @Description 远程方法调用的包装工具类
 * @Date 7/4/2026 下午1:10
 * @Author  Leo
 */
public class RemoteCallWrapper {
    ...
    private static final ImmutableSet<String> SUCCESS_CHECK_METHODS = ImmutableSet.of("isSuccess", "isSucceeded",
            "getSuccess");

    private static final ImmutableSet<String> SUCCESS_CODE_METHODS = ImmutableSet.of("getResponseCode");

    private static final ImmutableSet<String> SUCCESS_CODE = ImmutableSet.of("SUCCESS", "DUPLICATE",
            "DUPLICATED_REQUEST");
    ...
    private static <R> boolean isResponseValid(R response)
        ...
        for (Method method : methods) {
            ...
            if (SUCCESS_CHECK_METHODS.contains(methodName)){
    ...
    private static <R> boolean isResponseCodeValid(R response)
            throws IllegalAccessException, InvocationTargetException{
        Method successCodeMethod = null;
        Method[] methods = response.getClass().getMethods();
        for (Method method : methods) {
            String methodName = method.getName();
            if (SUCCESS_CODE_METHODS.contains(methodName)){
                successCodeMethod = method;
                break;
            }
        }
        if (successCodeMethod == null){
            return true;
        }

        return SUCCESS_CODE.contains((String) successCodeMethod.invoke(response));
    }
}
